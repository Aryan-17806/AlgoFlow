...
export default function PrefixPage() {
  ...
  const [status, setStatus] = useState("Ready to evaluate (Right-to-Left Scan)");
  ...
  const buildPrefixSteps = (exprStr) => {
    const frames = [];
    // Prefix expressions are scanned right-to-left
    ...
    let currentStack = [];
    // Line 6: Establish the reverse iteration baseline
    frames.push({ currentStack: [...currentStack], tIdx: parsedTokens.length, line: 6, msg: "Initializing right-to-left prefix scanning array pointer." });

    // Iterate backwards through the expression array tokens
    for (let i = parsedTokens.length - 1; i >= 0; i--) {
      const token = parsedTokens[i];
      
      // Line 8: Condition loop step evaluation block
      frames.push({ currentStack: [...currentStack], tIdx: i, line: 8, msg: `Scanning character token from right: "${token}"` });

      if (!isNaN(token)) {
        currentStack.push(Number(token));
        // Line 9: Push element operand onto execution stack
        frames.push({ currentStack: [...currentStack], tIdx: i, line: 9, msg: `Token "${token}" is an operand. Push onto stack.` });
      } else {
        ...
        // In Prefix, the first element popped is Operand 1 (left), the second is Operand 2 (right)
        const val1 = currentStack.pop();
        frames.push({ currentStack: [...currentStack], tIdx: i, line: 12, msg: `Operator found. Pop left parameter variable (${val1})` });
        
        const val2 = currentStack.pop();
        frames.push({ currentStack: [...currentStack], tIdx: i, line: 13, msg: `Pop right parameter variable (${val2})` });
        ...
        // Line 14: Push evaluated node metric step calculations
        frames.push({ currentStack: [...currentStack], tIdx: i, line: 14, msg: `Execute calculation: ${val1} ${token} ${val2} = ${result}. Push outcome.` });
      }
    }

    // Line 18: Termination return check
    if (currentStack.length === 1) {
      frames.push({ currentStack: [...currentStack], tIdx: -1, line: 18, msg: `Evaluation complete! Final prefix outcome computation = ${currentStack[0]}` });
    } else {
      frames.push({ currentStack: [...currentStack], tIdx: -1, line: 18, msg: "Evaluation complete. Warning: Stack contains dynamic residual entities." });
    }
  ...
  const runEvaluation = () => {
    ...
    const { frames, parsedTokens } = buildPrefixSteps(expression);
  ...
  const stepForward = () => {
    ...
    if (!s.length) {
      const res = buildPrefixSteps(expression);
      s = res.frames;
  ...
  const resetAll = () => {
    ...
    setStepIdx(-1);
    setCodeLine(0);
    setStatus("Ready to evaluate (Right-to-Left Scan)");
    setRunning(false);
  };
  ...
  return (
    <div className="visualizer-container">
      <div className="container-fluid p-4">
        ...
        <h2 className="title mb-4">Prefix Expression Stack Visualizer</h2>
          ...
          <div className="col-lg-7">
            <div className="card-dark p-3 mb-3">
              <div className="d-flex flex-wrap gap-2 align-items-center">
                <input
                  className="input-dark prefix-input"
                  type="text"
                  placeholder="e.g. - * + 5 3 2 7"
                  value={expression}
            ...
            <div className="card-dark p-4 text-center">
              ...
              <div className="staging-container mb-4 p-2 rounded">
                <div className="staging-title">EXP TOKENIZATION SCANNER STREAM (RIGHT-TO-LEFT SCAN)</div>
                <div className="d-flex justify-content-center align-items-center dynamic-stage-box gap-2">
                  {tokens.length === 0 ? (
                    ...
                    tokens.map((token, idx) => (
                      <span 
                        key={idx} 
                        className={`token-badge ${idx === tokenIdx ? "token-active-highlight" : idx > tokenIdx && tokenIdx !== -1 ? "token-passed" : ""}`}
                      >
                        {idx === tokenIdx && <span className="scan-direction-arrow">←</span>}
                        {token}
                      </span>
                    ))
                  )}
            ...
            <div className="card-dark p-3 mb-3">
              ...
              <div className="compiler-code-screen">
                {[
                  ...
                  "    stack<int> st;",
                  "    // Reiterate vector contents backwards",
                  ...
                  // Offset mapping coordinates to tie into sequential steps cleanly
      ...
      <style>{`
        ...
        .prefix-input { width: 220px; text-align: left; font-family: monospace; letter-spacing: 1px; }
        ...
        .token-badge { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; background: #161b22; border: 1px solid #30363d; border-radius: 4px; font-family: monospace; font-size: 14px; color: #c9d1d9; font-weight: bold; transition: all 0.2s ease; }
        ...
        .scan-direction-arrow { color: black; font-weight: 900; animation: blink 1s infinite; }
